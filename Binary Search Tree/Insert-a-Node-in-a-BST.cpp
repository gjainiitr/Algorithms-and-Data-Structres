/*
	Problem URL :- https://practice.geeksforgeeks.org/problems/insert-a-node-in-a-bst/1
*/
#include <bits/stdc++.h>
using namespace std;
...
void insertNode(Node *& node, int data) {
    ...
    if(node->data < data) {
        if(node->right==NULL) node->right = newNode(data);
        else insertNode(node->right,data);
    } else {
        if(node->left==NULL) node->left = newNode(data);
